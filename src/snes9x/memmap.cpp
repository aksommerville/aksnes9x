...
bool8 CMemory::LoadROMInt (int32 ROMfillSize)
{
	...
	// aks: I really don't know what any of this means, but Mickey and Donald 3 only works if we Deinterleave here.
	// Scanned my collection, and confirmed that nothing else matches the length and 8-byte prefix of that game.
	// There is surely a more correct way to decide this!
	if ((ROMfillSize==2097152)&&!memcmp(ROM,"\x4b\xab\xa0\x00\x00\xbe\x48\x82",8)) {
	  printf("Deinterleaving ROM because it looks like Mickey and Donald 3.\n");
  	S9xDeinterleaveType1(ROMfillSize, ROM);
 }
	...
	if (!Settings.ForceNotInterleaved && interleaved && (CalculatedSize >= 0x400000))
	{
	...
	if (tales && (CalculatedSize >= 0x400000))
	{
